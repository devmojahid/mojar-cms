<li class="jw-media-item" title="{{ $item->name }}">
    ...
    >
        ...
        <div class="jw-media-name text-truncate mt-1">
            <span>{{ $item->name }}</span>
        </div>
    </a>
</li>
...
{{-- <li class="media-item" title="{{ $item->name }}">
    <a href="{{ $item instanceof \Juzaweb\Backend\Models\MediaFolder ? route('admin.media.folder', [$item->id]) : 'javascript:void(0)' }}"
        class="media-item-info @if ($item instanceof \Juzaweb\Backend\Models\MediaFile) media-file-item @endif" data-id="{{ $item->id }}">
        @php
            $arr = $item->toArray();
            $arr['url'] = get_full_url(upload_url($item->path), url('/'));
            $arr['updated'] = jw_date_format($item->updated_at);
