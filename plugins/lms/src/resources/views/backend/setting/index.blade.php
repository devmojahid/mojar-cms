...
@section('content')
    <div class="container-xl lms-settings">
        <div class="page-header d-print-none">
            <div class="row align-items-center">
                <div class="col">
                    <h2 class="page-title">{{ $title ?? trans('lms::content.setting') }}</h2>
                </div>
            </div>
        </div>
        ...
        <form method="post" action="{{ route('admin.lms.setting.save') }}" class="form-ajax">
            @component('cms::components.tabs', [
                'tabs' => [
                    ...
                    'students' => [
                        'label' => trans('lms::content.students'),
                        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-users"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 7m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" /><path d="M3 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /><path d="M21 21v-2a4 4 0 0 0 -3 -3.85" /></svg>',
                        'show' => false
                    ],
                    'instructors' => [
                        'label' => trans('lms::content.instructors'),
                    ...
                    'certificates' => [
                        'label' => trans('lms::content.certificates'),
                    ...
                    'emails' => [
                        'label' => trans('lms::content.emails'),
                        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-mail"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 7a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-10z" /><path d="M3 7l9 6l9 -6" /></svg>',
                        'show' => false
                    ],
                    'pages' => [
                        'label' => trans('lms::content.pages'),
                ...
                @slot('tab_courses')
                    @include('lms::backend.setting.components.setting.courses')
                @endslot
              
                @slot('tab_students')
                    @include('lms::backend.setting.components.setting.students')
                @endslot
                
                @slot('tab_instructors')
                    @include('lms::backend.setting.components.setting.instructors')
                @endslot
                
                @slot('tab_certificates')
                    @include('lms::backend.setting.components.setting.certificates')
                @endslot
                
                @slot('tab_emails')
                    @include('lms::backend.setting.components.setting.emails')
                @endslot
                
                @slot('tab_pages')
                    @include('lms::backend.setting.components.setting.pages')
                @endslot 
            @endcomponent
