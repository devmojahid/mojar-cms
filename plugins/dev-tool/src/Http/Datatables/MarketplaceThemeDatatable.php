...
class MarketplaceThemeDatatable extends DataTable
{
    /**
    ...
    {
        $themes = MarketplaceTheme::whereIn('id', $ids)->get();
        ...
        switch ($action) {
            ...
            case 'featured':
                foreach ($themes as $theme) {
                    $theme->update(['is_featured' => true]);
                }
                break;
            case 'unfeatured':
                foreach ($themes as $theme) {
                    $theme->update(['is_featured' => false]);
                }
                break;
        }
    }
    ...
    public function getBulkActions(): array
    {
        return [
            'active' => [
                ...
                'action' => 'active',
                'confirm' => trans('Are you sure you want to activate these themes?'),
            ],
            'inactive' => [
                'label' => trans('Set Inactive'),
                'action' => 'inactive',
                'confirm' => trans('Are you sure you want to deactivate these themes?'),
            ],
            'featured' => [
                'label' => trans('Set Featured'),
                'action' => 'featured',
                'confirm' => trans('Are you sure you want to set these themes as featured?'),
            ],
            'unfeatured' => [
                'label' => trans('Unset Featured'),
                'action' => 'unfeatured',
                'confirm' => trans('Are you sure you want to unset featured status for these themes?'),
            ],
            'delete' => [
                'label' => trans('Delete'),
                'action' => 'delete',
                'confirm' => trans('Are you sure you want to delete these themes?'),
            ],
        ];
    }
    ...
    public function actions(): array
    {
        return [
            'edit' => [
                'label' => trans('Edit'),
                'url' => function ($item) {
                    return route('admin.dev-tool.marketplace-themes.edit', [$item->id]);
                },
                'icon' => 'fa fa-edit',
            ],
            'delete' => [
                ...
                'action' => 'delete',
                'url' => function ($item) {
                    return route('admin.dev-tool.marketplace-themes.destroy', [$item->id]);
                },
                'confirm' => trans('Are you sure you want to delete this theme?'),
                'icon' => 'fa fa-trash',
            ],
        ];
    }
